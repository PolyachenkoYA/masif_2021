...
def get_lin_features(pdb_filepath, Ravg, to_comp_cor=False, main_mode=0, N_centers=200, chain_vec_avg=1, \
	...
	features_filepath = '_'.join([pdb_filepath_base, \
							      'features', \
								  'R' + str(Ravg), \
	...
	if(to_recompute or (not os.path.isfile(features_filepath)) or to_draw_anything):
		...
		main_sections_areas = np.ones(N_centers)
		main_sections_convex_areas = np.ones(N_centers)
		...
		if(regular_trimesh.is_watertight):
			...
			print('mesh is not watertight, sections will not be computed')
