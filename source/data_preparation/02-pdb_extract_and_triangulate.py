...
def get_R_cor_sq(feature, main_mode):
	...
	for f1_i in range(N_feat):
		...
		for f2_i in range(f1_i + 1, N_feat):
			...
			R_cor[f1_i, f2_i], _ = scipy.stats.pearsonr(feature[main_mode, :, f1_i], feature[main_mode, :, f2_i])
...
def main():
	...
	#center_coords_residues = np.zeros((N_center_types, N_resd, 3))
	center_coords_residues = []
	center_coords_residues.append(molecule.xyz[0, molecule.topology.select('name CA'), :])   # C_A coords
	center_coords_residues.append(np.zeros((N_resd, 3)))   # center of mass coords
	for res_i in range(N_resd):   # center of mass coords
		center_coords_residues[1][res_i, :] = np.mean(molecule.xyz[0, molecule.topology.select('resid == ' + str(res_i)), :], axis=0)   # center of mass coords
	center_coords_residues.append(molecule.xyz[0, molecule.topology.select('backbone'), :])   # backbone_coords

	### =================== construct paths =======================
	N_centers = N_resd * N_interp_scale
	center_coords = np.zeros((N_center_types, N_centers, 3))
	chain_vectors = []
	center_resd_indices = np.zeros((N_center_types, N_centers), dtype=int)
	for center_type_i in range(N_center_types):
		center_coords[center_type_i, :, :], center_resd_indices[center_type_i, :], chain_vectors_local = \
	...
	for R_i in range(N_R):
		R_cor[R_i, :, :] = get_R_cor_sq(get_lin_features_from_vertices(vertices, center_coords, vertices_features, R_arr[R_i]), main_mode)
	...
	if(to_draw_centers or to_draw_atoms or to_draw_vertices):
		...
		if(to_draw_centers):
			#for center_type_i in range(N_center_types):
	# 			ax_protein.plot3D(center_coords_residues[center_type_i, :, 0], center_coords_residues[center_type_i, :, 1], center_coords_residues[center_type_i, :, 2], \
	# 					 label='C:' + center_coords_names[center_type_i])
	 		ax_protein.plot3D(center_coords[main_mode, :, 0], center_coords[main_mode, :, 1], center_coords[main_mode, :, 2], \
	...
	if(to_plot_features):
		...
		for f_i in range(N_feat):
			#ax_feat.plot(sq_nums, lin_feauture_sq_u[main_mode, :, f_i], '--o', markersize=3, label=features_names[f_i])
			ax_feat.scatter((np.arange(N_centers) + 1) * (N_resd / N_centers), lin_feauture_sq_u[main_mode, :, f_i], s=2, label=features_names[f_i])
